...
class Video2Picture(object):
    ...
    def __init__(self, videoRoute='', saveRoute='', frameRate=1):
        """
        初始化对象
        :param videoRoute: 视频文件路径
    ...
    def video2Picture(self):
        videoCapture = cv2.VideoCapture(self.videoRoute)
        i = 0
        j = 0
        ...
        while success:
            ...
            if i % self.frameRate == 0:
                frameRoute = os.path.join(self.saveRoute, (str(j) + '.jpg'))
                cv2.imwrite(frameRoute, frame)
                j = j + 1
                print('图片保存地址：', frameRoute)
            success, frame = videoCapture.read()
        ...
        if self.folderRoute != '':
            ...
            for name in os.listdir(self.folderRoute):
                ...
                if fileSuffix != '':
                    ...
                    continue
            return self.saveRoute, saveRoute
        else:
...
class CutBmp(object):
    ...
    def section_Folder(self, row, col, rowOffset, colOffset, show=False, save=False):
        """
        使用切片法将BMP图像裁剪为合适的大小
        :param row: 切片后行数
        :param col: 切片后列数
        ...
        if self.folderRoute != '':
            ...
            for name in os.listdir(self.folderRoute):
                ...
                if fileSuffix != '':
                    bmpRoute = os.path.join(self.folderRoute, name)

                    bmp_mat = mpimg.imread(bmpRoute)
                    bmpRow = bmp_mat.shape[0]
                    bmpCol = bmp_mat.shape[1]
                    ...
                    if save:
                        ...
                        newBmp_mat.save(saveRoute[saveRouteIndex])
                        saveRouteIndex += 1
                    else:
                        return None
                else:
                    print('CutBmp.section_Folder ERROR 1')
                    continue
            return self.saveRoute, saveRoute
        else:
            print('CutBmp.section_Folder ERROR 0')
            return None

    def resize(self, row, col, show=False, save=False):
        """
        使用Image.resize将BMP图像裁剪为合适的大小
        :param row: 切片后行数
        :param col: 切片后列数
        :param show: 是否显示
        :param save: 是否保存
        ...
        bmp_mat = Image.open(self.bmpRoute)
        bmpCol, bmpRow = bmp_mat.size

        # 判断是否适合修改，若适合，则进行修改
        if (bmpRow >= row) and (bmpCol >= col):
            newBmp_mat = bmp_mat.resize((col, row))
        else:
            print('CutBmp.section ERROR 0')
            return None
    ...
    def resize_Folder(self, row, col, show=False, save=False):
        """
        使用Image.resize将BMP图像裁剪为合适的大小
        :param row: 切片后行数
        :param col: 切片后列数
        ...
        if self.folderRoute != '':
            ...
            for name in os.listdir(self.folderRoute):
                ...
                if fileSuffix != '':
                    ...
                    print('CutBmp.resize_Folder ERROR 1')
                    continue
            return self.saveRoute, saveRoute
        else:
            print('CutBmp.resize_Folder ERROR 0')
            return None
...
class PrintText(object):
    ...
    def printText_Folder(self, startPoint=(0, 0), txtPage=8, txtCol=128, txtPageBits=8, show=False, save=False):
        """
        将黑白图输出成十六进制文本
        :param startPoint: 起始取模坐标点，两个分量都大于零
        :param txtPage: 生成的TXT的页数
        ...
        if self.folderRoute != '':
            ...
            for name in os.listdir(self.folderRoute):
                ...
                if fileSuffix != '':
                    if (startPoint[0] < 0) or (startPoint[1] < 0):
                        print('PrintText.printText ERROR 4')
                        continue
                    ...
                    if save:
                        text = np.array(text)
                        saveRoute.append(os.path.join(self.saveRoute, (name + '.txt')))
                        np.savetxt(fname=saveRoute[saveRouteIndex], X=text, fmt='0X%X,',
                                   delimiter='\t', newline='\n',)
                        saveRouteIndex += 1
                else:
                    print('PrintText.printText ERROR 1')
